...
const PrimaryBackgroundContainer = tw(
  Container
)`-mx-8 px-8 bg-transparent text-gray-100`;

const HeadingContainer = tw.div`text-white`;
const Subheading = tw(SubheadingBase)` text-center text-gray-100 mb-4 `;
const Heading = tw(SectionHeading)`text-white pt-0`;
const Description = tw(SectionDescription)` mx-auto text-center text-white`;

const TestimonialsSlider = styled(Slider)`
  ${tw`flex mt-16 mx-auto max-w-xs sm:max-w-xl lg:max-w-4xl text-left bg-transparent rounded-lg text-gray-900 `}
  .slick-track {
    ${tw`flex!`}
  }
  .slick-slide {
...
`;
const Testimonial = tw.div`text-white px-6 py-12 sm:px-20 sm:py-16 bg-gray-800 rounded-2xl `;
const QuoteContainer = tw.div`relative text-white`;
const QuoteIcon = tw(
  QuoteIconBase
)`text-white absolute opacity-15 top-0 left-0 transform -translate-y-2 -translate-x-1/2 sm:-translate-x-full w-10 fill-current text-primary-500`;
const Quote = tw.blockquote`text-white font-medium sm:font-normal relative text-sm sm:text-xl text-center sm:text-left`;
...
const CustomerName = tw.span`text-lg font-semibold text-white`;
