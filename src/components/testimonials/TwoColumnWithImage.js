...
const Container = tw.div`relative bg-gray-200 mt-4` ; //chenge no bg to gray bg,add maring-4  14-2-24
...
const CustomerName = tw.h5`font-bold text-lg lg:text-xl xl:text-2xl text-green-400`;//chenge bg primary to green on 14-2-24
const CustomerTitle = tw.p`font-medium text-sm`;

const QuotesLeft = tw(QuotesLeftIcon)`w-8 h-8 lg:w-10 lg:h-10 text-green-400 absolute top-0 left-0`;//chenge bg primary to green on 14-2-24
const QuotesRight = tw(QuotesRightIcon)`w-8 h-8 lg:w-10 lg:h-10 text-green-400 absolute bottom-0 right-0`;//chenge bg primary to green on 14-2-24
