...
import { ReactComponent as PinIcon } from "feather-icons/dist/icons/bell.svg";
...
const Hovereffect=tw.div`hover:(scale-102 duration-500 shadow-2xl)`


const Icone=tw.img`w-12 h-12 rounded-full mt-8 ml-12 `

const Container = tw.div`relative bg-gray-200 `;
const Content = tw.div`max-w-screen-xl mx-auto py-16 lg:py-20`;

const HeadingWithControl = tw.div`flex flex-col items-center sm:items-stretch sm:flex-row justify-between`;
const Heading = tw(SectionHeading)`ml-8`;
...
`;
const PrevButton = tw(ControlButton)`bg-green-600`;
const NextButton = tw(ControlButton)`bg-green-600`;
...
`;
const Card = tw.div`h-full flex! flex-col bg-white sm:border max-w-sm sm:rounded-tl-4xl sm:rounded-br-5xl relative focus:outline-none`;
          ...
          { 
           products.map((product, index) => (
            ...
            <Card key={index}>
              ...
              <Icone src={product.profileImageSrc}/>
              <TextInfo>
