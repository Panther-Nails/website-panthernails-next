...
export default ({ children, properties, index }) => {
  ...
  return (
    <>
      <Plan key={index} featured={properties.featured}>
        <PlanHeader>
          <span className="nameAndFeaturedContainer">
            <span className="name">{properties.name}</span>
            {properties.featured && (
              <span className="featuredText">{properties.featured}</span>
            )}
          </span>
          <div className="pricingContainer">
            <span className="currentPrice">
              <span className="bigText">{price[0]}</span>
              {price[1]}{" "}
            </span>
            {properties.oldPrice && (
              <span className="oldPrice">{properties.oldPrice}</span>
            )}
          </div>
        ...
        <PlanAction>
          <ActionButton as="a" href={properties.buttonUrl}>
