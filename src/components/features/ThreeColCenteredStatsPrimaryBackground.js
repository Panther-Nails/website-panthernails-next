...
import { Container as ContainerBase, ContentWithPaddingForHomePage } from "components/misc/Layouts";
import { SectionDescription } from "components/misc/Typography";
 
const Container = tw(ContainerBase)`my-8 lg:my-10  text-gray-100 -mx-8 px-8 bg-blue-300 mr-0 ml-0`;
...
const StatValue = tw(animated.div)`text-4xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-black font-bold`;
...
export default ({
  ...
  const animatedProps = useSpring({
    displayValue: inView ? animatedStats.reduce((acc, stat) => acc + stat.value, 0) : 0,
    from: { displayValue: 0 },
